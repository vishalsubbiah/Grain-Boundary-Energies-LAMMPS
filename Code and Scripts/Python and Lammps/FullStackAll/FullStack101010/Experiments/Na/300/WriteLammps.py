...
def write_lammps(M1,M2,potential,element,temp,tempDamp,theta,lattice,minEng,typeAtom):
...
lattice """+str(typeAtom)+""" """+str(lattice)+""" 
region whole block -10 10 -10 10 -10 10 units lattice 
create_box 2 whole 
region upper block INF INF 0 10 INF INF units lattice 

lattice """+str(typeAtom)+""" """+str(lattice)+""" orient x """ +str(x1[0])+" " + str(x1[1]) + " " + str(x1[2]) +""" orient y  """+ str(y1[0])+" " + str(y1[1]) + " " + str(y1[2]) +""" orient z """ +str(z1[0])+" " + str(z1[1]) + " " + str(z1[2]) +"""\n""" 


    ContentAtomStruct2="""\n create_atoms 1 region upper 
region lower block INF INF -10 0.000000 INF INF units lattice 
lattice """+str(typeAtom)+""" """+str(lattice)+""" orient x """ +str(x2[0])+" " + str(x2[1]) + " " + str(x2[2]) +""" orient y  """+ str(y2[0])+" " + str(y2[1]) + " " + str(y2[2]) +""" orient z """ +str(z2[0])+" " + str(z2[1]) + " " + str(z2[2])  +"""\n"""
...
delete_atoms overlap 0.35 lower upper \n"""
    ...
    ContentSettings="""\n # ---------- Define Settings --------------------- 
compute csym all centro/atom """+str(typeAtom)+"""
...
reset_timestep	0
timestep 0.001
velocity all create """+str(temp)+""" 12345 mom yes rot no
...
minimize 1e-15 1e-15 5000 5000 \n"""
    ...
    ContentGBEng="""\n # ---------- Calculate GB Energy --------------------- 
variable minimumenergy equal """+str(minEng)+"""
...
write_dump all custom dump.img_post_minimize_"""+str(theta)+""".cfg mass type xs ys zs
print "All done" \n"""
